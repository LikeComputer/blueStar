...
public class JTopBehave extends CoordinatorLayout.Behavior<View> {
    ...
    public JTopBehave(Context context, AttributeSet attrs){
        super(context, attrs);
    ...
    public void onStopNestedScroll(CoordinatorLayout coordinatorLayout, View child, View target){
        ...
        // target.getTag() recycleview滑动的距离 上滑动>0
        if(mDyUnconsumeds == 0) {
            //targetview 可滑动的时候
            ...
            if(mTitleHide) {
                //隐藏title
                if(dyConsumed>=mScaledTouchSlop*2) {
                    //滑动太快 动画隐藏
                    hide(child);
                }else if(Math.abs(child.getTranslationY())<=child.getBottom()/2 && verticalScrollOffset<paddingTop) {
//                    出屏幕不超过2/3就显示
                    show(child);
                }
            }else {
