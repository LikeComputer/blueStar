...
public class ToastHelper {
    ...
    public ToastHelper showToast(Object msg) {
        if (CheckHelper.safeObjects(msg)) {
    ...
    public static Toast showShort(CharSequence message, @ColorInt int bgcolor) {
        Toast toast = null;
        if (CheckHelper.safeObjects(message)) {
    ...
    /**
     * @param duration 1-LONG,0-SHORT
     * @param colors BGcolor,MSGcolor
     */
    public static TextView showToast(Object message, int duration, @ColorInt Integer... colors) {
        if (CheckHelper.safeObjects(message)) {
            ...
            if (CheckHelper.safeArrays(colors)) {
                if (colors[0] != DEFAULT) {
    ...
    /**
     * @param res BG resource,MSGcolor
     */
    ...
    public static void toastLongRes(int resId) {
        if (CheckHelper.safeObjects(resId)) {
    ...
    public static void toastErrorMsg(Throwable throwable) {
        ToastHelper.showShort(getMessage(throwable));
    }


    public static String getMessage(Throwable throwable) {
        String message = throwable.getMessage();
        if (TextUtils.isEmpty(message) || throwable instanceof IOException || throwable instanceof SocketTimeoutException ||
                throwable instanceof UnknownHostException || throwable != null && !TextUtils.isEmpty(message = throwable.getMessage())) {
            message = "当前网络异常，请先检查网络";
        }
        return message;
    }


    @Nullable public static TextView getTextViewFromToast(@Nullable Toast toast) {
        if (toast != null) {
