...
public class NumHelper {
    ...
    /**
     * @param num {精确到两位小数 4.01---5，4.001---4}
     * @return 最接近num的数 同时这个数是round的倍数
     */
    public static int getRound(float num,int round){
        return ( (int)( num+round/2f-0.01 ) )/round*round;
    }

    /**
     * 如果是小数的话 num,round都乘以10变整数，然后除以10
     * @param num  {精确到两位小数 4.01---5，4.001---4}
     * @return 最接近num的数 同时这个数是round的倍数
     */
     ...
     */
    public static int getCell5(float num){
        return ( (int)( num+4.99f ) )/5*5;
    }
    ...
    public static float null2float(Object o){
        if(o == null) {
            return 0;
        }
        return (float)o;
