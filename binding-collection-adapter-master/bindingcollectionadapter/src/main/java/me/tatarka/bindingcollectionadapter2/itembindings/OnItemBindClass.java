 ...
 *     .regist(String.class, BR.name, R.layout.item_name)
 *     .regist(Footer.class, ItemBinding.VAR_NONE, R.layout.item_footer);
 * }</pre>
 */
public class OnItemBindClass<T> extends OnItemBind<T> {
     ...
     */
    public OnItemBindClass<T> regist(@NonNull Class<? extends T> itemClass, final int variableId, @LayoutRes final int layoutRes) {
     ...
     */
    public <E extends T> OnItemBindClass<T> regist(@NonNull Class<E> itemClass, OnItemBind<E> onItemBind) {
        int index = itemBindingClassList.indexOf(itemClass);
        if (index >= 0) {
    ...
    /**
     * Returns the number of item types in the regist. This is useful for {@link
    ...
    public void onItemBind(ItemBinding itemBinding, int position, T item) {
        for (int i = 0; i < itemBindingClassList.size(); i++) {
            ...
            if (key.isInstance(item)) {
                ...
                itemBinding.setExtraBindings(itemBind.getExtraBindings());
                return;
            }
        }
    ...
    private OnItemBind<T> itemBind(final int variableId, @LayoutRes final int layoutRes) {
        return new OnItemBind<T>() {
            @Override
            public void onItemBind(ItemBinding itemBinding, int position, T item) {
                itemBinding.set(variableId, layoutRes);//更新 layoutRes
            }
        };
    }
}
